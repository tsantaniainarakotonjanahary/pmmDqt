...
function Formulaire() 
{
  ...
  const handleClick = () => {
    ...
    setLoading(true);
    const url = "https://pmm-data-quality-teal1.vercel.app/" + erreur + "-" + sortie;
    getData(url,"Nosybe","2021@Covax",periode,orgUnit.value,sortie + "s",sortie.toUpperCase(),sortie + "Date").then((data) => {
      getData("https://pmm-data-quality-teal1.vercel.app/doublon-enrollment","Nosybe","2021@Covax",periode,orgUnit.value,"enrollments","enrollment".toUpperCase(),"enrollment" + "Date" ).then((data) => { setDr(data.data.length); });
      getData("https://pmm-data-quality-teal1.vercel.app/NA-enrollment","Nosybe","2021@Covax",periode,orgUnit.value,"enrollments","enrollment".toUpperCase(),"enrollment" + "Date").then((data) => { setNr(data.data.length); });
      getData("https://pmm-data-quality-teal1.vercel.app/doublon-event","Nosybe","2021@Covax",periode,orgUnit.value,"events","event".toUpperCase(),"event" + "Date").then((data) => { setDv(data.data.length); });
      getData("https://pmm-data-quality-teal1.vercel.app/NA-event","Nosybe","2021@Covax",periode,orgUnit.value,"events","event".toUpperCase(),"event" + "Date").then((data) => { setNv(data.data.length); });
      setData(data);
      setLoading(false);
    });
  };
